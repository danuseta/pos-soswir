<script lang="ts">
  ...
  const navItems = [
    ...
    { href: '/cashier/products', label: 'Produk', icon: Package },
    { href: '/cashier/transactions', label: 'Transaksi', icon: FileText }
  ];
          ...
          <div class="flex-1 flex">
            <div class="hidden sm:flex items-center">
              <h2 class="text-lg font-semibold text-card-foreground">
                {navItems.find(item => isCurrentPage(item.href))?.label || 'Dashboard'}
              </h2>
            </div>
          </div>
          ...
          <div class="ml-4 flex items-center space-x-2 sm:space-x-4">
            ...
            <div class="relative profile-dropdown">
              ...
              {#if showProfileDropdown}
                <div class="origin-top-right absolute right-0 mt-2 w-56 rounded-lg shadow-lg py-1 bg-card ring-1 ring-border focus:outline-none z-50 border border-border">
                  <div class="px-4 py-3 text-sm text-muted-foreground border-b border-border">
                    <div class="font-medium text-card-foreground">{username}</div>
                    <div class="text-xs">Kasir</div>
                  </div>
                  <a
                    href="/cashier/profile"
                    class="flex items-center gap-2 px-4 py-2 text-sm text-card-foreground hover:bg-accent transition-colors"
